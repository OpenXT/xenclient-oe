...
def package_qa_check_license(workdir, d):
    ...
    for url in lic_files.split():
        ...
        if parm['md5'] == md5chksum:
            ...
            bb.warn (pn + ": md5 data is not matching for ", url)
            bb.warn (pn + ": The new md5 checksum is ", md5chksum)
            bb.warn (pn + ": Check if the license information has changed in")
            sane = False
...
Missing inherit gettext?""" % (gt, config))
    ...
    if not package_qa_check_license(workdir, d):
        bb.warn("Licensing Error: LIC_FILES_CHKSUM does not match, please fix")
}
